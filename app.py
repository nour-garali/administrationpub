...
from .models import db, AdminPublique, Users, Funds
...
def signup_admin():
    ...
    if file and allowed_file(file.filename):
        ...
        # Créer un nouvel utilisateur avec les données du formulaire et le nom de fichier
        new_user = AdminPublique(firstName=firstName, lastName=lastName, password=hashed_password,email=email, directeur =directeur ,profile_image=filename)
        db.session.add(new_user)
        db.session.commit()
